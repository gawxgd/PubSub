...
public class HandleSubscriberClientConnectionUseCase(Socket socket, Action onConnectionClosed, ICommitLogFactory commitLogFactory) : IHandleClientConnectionUseCase
{
    private readonly string _connectedClientEndpoint = socket.RemoteEndPoint?.ToString() ?? "Unknown";

    private static readonly IAutoLogger Logger =
        AutoLoggerFactory.CreateLogger<HandleSubscriberClientConnectionUseCase>(LogSource.MessageBroker);
