...
So that messages are not lost or duplicated when components restart
    ...
    Scenario: Delivery after publisher restart
        Given a publisher is configured with the following options:
          | Setting            | Value          |
          | Topic              | default        |
          | Broker             | 127.0.0.1:9096 |
    ...
    Scenario: Publisher continues sending messages during broker restart
        Given a publisher is configured with the following options:
          | Setting            | Value          |
          | Topic              | default        |
          | Broker             | 127.0.0.1:9096 |
        ...
        And a subscriber is configured with the following options:
          ...
          | Poll Interval      | 100            |
          | Max Retry Attempts | 3              |
        When the publisher sends 10 messages to topic "default" and the broker restarts after 3 messages
        Then subscriber C consumed messages up to offset 9
